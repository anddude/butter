...
export async function ensureKbSeeded(): Promise<void> {
    ...
    if (!pinecone) {  kbSeeded = true;  return; }
...
export async function searchKb( queryText: string, topK: number ):  Promise<{ matches: RagMatch[]; contextText: string; contextChars: number }> {
    const pinecone = getPinecone();
