...
function getLlmClient(): { client: OpenAI; model: string } {
  ...
  if (!cachedClient || cachedProvider !== llmProvider) {
    if (llmProvider === 'openrouter') {
      // OpenRouter uses OpenAI SDK with baseURL + its own key
...
function buildPrompt(args: {
  ...
  const task =
    "Your job is to summarize the user's text which is an email and produce actionable next steps for the user.";
  ...
  const baseRequirements =
    ...
    '3) Ignore anything that is not related to work: pleasantries, greetings, small talk, personal conversations, personal information.' +
    '4) Focus on the topic of the company projects, the specific client needs being discussed, and the workflow being discussed.' +
    '5) Use the provided top keywords as signals for emphasis, but do not invent facts. ' +
    '6) If the Top keywords signal the email sender was off topic. Ignore the top keywords when making the summary. You should only summarize necessary work items.' +
  ...
  const requirements = [
    baseRequirements,
    args.ragContext ? ragRequirements : null,
  ]
    .filter(Boolean)
    .join('\n');
...
export async function ragSummarizeText(
  rawBody: unknown,
